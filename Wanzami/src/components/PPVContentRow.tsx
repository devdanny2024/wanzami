...
import { PPVMovieCard, PPVMovieData } from './PPVMovieCard';
import { motion } from 'motion/react';

interface PPVContentRowProps {
  title: string;
  movies: PPVMovieData[];
  onMovieClick: (movie: PPVMovieData) => void;
}

export function PPVContentRow({ title, movies, onMovieClick }: PPVContentRowProps) {
        ...
        >
          {movies.map((movie) => (
            <div key={movie.id} className="w-[280px] md:w-[320px] lg:w-[380px]">
              <PPVMovieCard movie={movie} onClick={onMovieClick} />
            </div>
          ))}
        </div>
