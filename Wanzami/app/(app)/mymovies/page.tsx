    ...
    return (
      ...
      >
        <div className="relative aspect-video rounded-xl overflow-hidden bg-graphite border border-white/10 group-hover:border-brand/60 transition-colors">
          ...
          {isExpired && (
            <div className="absolute top-2 left-2 text-xs px-2 py-1 rounded bg-black/70 border border-white/10 text-ash">
              Expired
            </div>
          )}
          {!isExpired && expiresLabel && (
            <div className="absolute top-2 left-2 text-xs px-2 py-1 rounded bg-brand/20 border border-brand/50 text-brand-light">
              Expires {expiresLabel}
            </div>
          )}
        </div>
        <div className="mt-2">
          <p className="text-foreground text-sm font-semibold line-clamp-1">{item.title.name}</p>
  ...
  return (
    <div className="min-h-screen bg-background pt-28 md:pt-32 pb-12 container-page home-root">
      <div className="flex items-end justify-between flex-wrap gap-4 mb-8">
        <div>
          <h1 className="font-heading text-foreground text-5xl md:text-6xl tracking-wide uppercase">My Movies</h1>
          <p className="text-ash mt-2">Your purchased titles and access windows.</p>
        </div>
        {active.length > 0 && (
      ...
      {loading && <p className="text-ash">Loading your purchases...</p>}
      ...
      {!loading && !error && active.length > 0 && (
        <div className="space-y-4 mb-10">
          <h2 className="font-heading text-foreground text-2xl tracking-wide uppercase">Active purchases</h2>
      ...
      {!loading && !error && expired.length > 0 && (
        <div className="space-y-4">
          <h2 className="font-heading text-foreground text-2xl tracking-wide uppercase">Previously purchased</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-4">
