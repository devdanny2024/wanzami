...
export const ingestEvents = async (req, res) => {
    ...
    const profileIds = Array.from(new Set(events
    ...
    for (const e of events) {
        ...
        const experiment = req.headers["x-experiment"] ?? undefined;
        const variant = req.headers["x-variant"] ?? undefined;
        const baseMetadata = e.metadata ?? undefined;
        const metadata = experiment || variant
            ? {
...
export const adminEventsSummary = async (req, res) => {
    ...
    const counts = countsRaw.map((c) => ({
        ...
        count: typeof c._count === "object" && c._count ? c._count._all ?? 0 : 0,
    }));
    ...
    const recentMapped = recent.map((e) => {
        ...
        const completion = typeof metadata.completionPercent === "number"
