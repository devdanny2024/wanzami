...
export const createMultipartUpload = async (key, contentType = "application/octet-stream") => {
    if (!config.s3.bucket) {
...
export const presignPartUrls = async (key, uploadId, partCount) => {
    if (!config.s3.bucket) {
        throw new Error("S3 bucket not configured");
    }
    const client = s3Client();
...
export const uploadFile = async (key, filePath, contentType = "video/mp4") => {
    if (!config.s3.bucket) {
        throw new Error("S3 bucket not configured");
    }
    const client = s3Client();
...
export const downloadToFile = async (key, destPath) => {
    if (!config.s3.bucket) {
        throw new Error("S3 bucket not configured");
    }
    const client = s3Client();
...
export const presignPutObject = async (key, contentType = "application/octet-stream", expiresIn = 900) => {
    if (!config.s3.bucket) {
        throw new Error("S3 bucket not configured");
    }
    const client = s3Client();
