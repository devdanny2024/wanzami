...
const ensureProfileExists = async (userId, name) => {
...
export const googleAuthUrl = async (redirectUri) => {
...
export const googleAuthCallback = async (input) => {
    ...
    if (!tokenRes.ok) {
        ...
        throw new Error(err?.error || "Google token exchange failed");
    }
    ...
    const rawPrefs = (primaryProfile.preferences ?? {});
    const hasPreferredGenres = Array.isArray(rawPrefs.preferredGenres) && rawPrefs.preferredGenres.length > 0;
